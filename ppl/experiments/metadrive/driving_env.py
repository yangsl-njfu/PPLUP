...
import numpy as np
from ppl.experiments.metadrive.base_prediction_env import BasePredictionEnv
...
class DrivingEnv(BasePredictionEnv):
    ...
    def reset(self, *args, **kwargs):
        ...
        if "force_seed" in kwargs:
            force_seed = kwargs.pop("force_seed")
            if "seed" in kwargs and kwargs["seed"] != force_seed:
                raise ValueError("Received both force_seed={} and seed={}".format(force_seed, kwargs["seed"]))
            kwargs["seed"] = force_seed

        try:
            reset_ret = super(DrivingEnv, self).reset(*args, **kwargs)
        except TypeError as seed_error:
            if not _is_unexpected_reset_kw_error(seed_error, "seed") or "seed" not in kwargs:
                raise
            force_seed = kwargs.pop("seed")
            kwargs["force_seed"] = force_seed
            reset_ret = super(DrivingEnv, self).reset(*args, **kwargs)

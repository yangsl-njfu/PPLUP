...
def _is_unexpected_reset_kw_error(error, keyword):
    message = str(error)
...
class RecorderEnv(Wrapper):
    ...
    def reset(self, force_seed=None, seed=None, **kwargs):
        if force_seed is not None and seed is not None and force_seed != seed:
            raise ValueError("Received both force_seed={} and seed={}".format(force_seed, seed))

        reset_seed = seed if seed is not None else force_seed
        if reset_seed is None:
            o = super(RecorderEnv, self).reset(**kwargs)
        else:
            force_seed_kwargs = kwargs.copy()
            force_seed_kwargs["force_seed"] = reset_seed
            try:
                o = super(RecorderEnv, self).reset(**force_seed_kwargs)
            except TypeError as force_seed_error:
                if not _is_unexpected_reset_kw_error(force_seed_error, "force_seed"):
                    raise

                seed_kwargs = kwargs.copy()
                seed_kwargs["seed"] = reset_seed
                try:
                    o = super(RecorderEnv, self).reset(**seed_kwargs)
                except TypeError as seed_error:
                    if _is_unexpected_reset_kw_error(seed_error, "seed"):

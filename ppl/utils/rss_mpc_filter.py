...
class RSSMPCFilter(RSSCBFFilter):
    ...
    def filter_action(self, state: State, u_nom: Sequence[float]) -> Tuple[Action, Dict[str, Any]]:
        ...
        def _diagnostics_block() -> Dict[str, Any]:
            ...
            return {
                ...
                "first_acc": selected_diag.get("first_acc", math.nan),
                ...
                "road_boundary_safe": bool(selected_diag.get("road_boundary_safe", lateral_stats["road_safe"])),
    ...
    def _evaluate_lateral_escape_gate(self, state: State, evaluation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif not (terminal_lateral_safe or lateral_margin_improved or path_overlap_reduced or lateral_terminal_reason):
            reject_reason = "lateral_escape_lateral_rss_unsafe"
        elif not lateral_certified_gate:
    ...
    def _check_certified_lateral_escape(
        ...
        if not bool(evaluation.get("lateral_certified_gate", False)):
            return False, "", ""
        ...
        if min_road_boundary < self.mpc_config.corridor_road_boundary_margin:
            return False, "", ""
        if float(u_mpc[0]) <= 0.0 or float(u_guarded[0]) > self.config.small_tolerance:
            return False, "", ""
